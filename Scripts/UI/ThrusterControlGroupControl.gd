...
signal thruster_control_group_setting_changed(value)

func _gui_input(event):
	if (event is InputEventMouseButton) && !event.pressed && (event.button_index == BUTTON_LEFT):
		_emit_control_group_setting_changed()
...
func _emit_control_group_setting_changed():
	emit_signal("thruster_control_group_setting_changed", value)
